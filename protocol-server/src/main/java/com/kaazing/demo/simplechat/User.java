...
public class User {
	...
	public String toString() {
		return String.format("%s[address=%s,username=%s,role=%s]", this.getClass().getSimpleName(), remoteAddress,
				username, role);
	}
	...
	public void processNewBytes(byte[] bytes) {
		...
		if (currentMessage == null) {
			...
			if (remainingBytes.length >= len) {
				...
				LOGGER.debug(
						String.format("%s Got some of the bytes: [%s]", loggerId, Util.bytesToString(messageBytes)));
			...
			LOGGER.info(String.format("%s Continuing an existing message. Number of bytes still needed: %d", loggerId,
					remainingLen));
	...
	private void processCredentialsMessage(JSONObject message) {
		...
		case customer:
            ...
            // otherwise there are agents, tell them that a new customer has arrived.
    ...
    public void sendUserConnectedMessage(User newUser) {
        LOGGER.info(String.format("%s Notifying %s (%s) that %"
                + "s (%s) is present", loggerId, username, role, newUser.username, newUser.role));
//        String str = "{\"type\":\"connected\",\"username\":\"" + newUser.getUsername() + "\",\"role\":\""
//                + newUser.role + "\"}"; // TODO: Delete
        String str = String.format("{\"type\":\"connected\",\"username\":\"%s\",\"role\":\"%s\"}", newUser.getUsername(), newUser.role);
    ...
    public void sendUserDisconnectedMessage(User user) {
        LOGGER.info(String.format("%s Notifying %s (%s) that %"
                + "s (%s) disconnected", loggerId, username, role, user.username, user.role));
        //String str = "{\"type\":\"disconnected\",\"username\":\"" + user.getUsername() + "\"}"; TODO: Delete
...
//		String str = "{\"type\":\"customerMessage\",\"sender\":\"" + sender.getUsername() + "\",\"messageText\":\""
//				+ messageText + "\"}"; TODO: Delete
        String str = String.format("{\"type\":\"customerMessage\",\"sender\":\"%s\",\"messageText\":\"%s\"}", sender.getUsername(), messageText);
		String strLen = Util.padLeftZeros(String.valueOf(str.length()), LENGTH_PREFIX_LENGTH);
...
//		String str = "{\"type\":\"agentMessage\",\"sender\":\"" + sender.getUsername() + "\",\"\":\""+""\",\"messageText\":\""
//				+ messageText + "\"}"; // TODO: Delete
		String str = String.format("{\"type\":\"agentMessage\",\"sender\":\"%s\",\"to\":\"%s\",\"messageText\":\"%s\"}", sender.getUsername(), recipient.username, messageText);
		String strLen = Util.padLeftZeros(String.valueOf(str.length()), LENGTH_PREFIX_LENGTH);
        LOGGER.info(String.format("%s Sending message to %s", loggerId, username));
		LOGGER.info(String.format("%s message=%s len=%s", loggerId, str, strLen));
	...
	public void sendBlah() {
		byte[] testMessage1 = ("hello").getBytes(StandardCharsets.UTF_8);
		server.send(socket, testMessage1);
	}

	private void sendTestMessages() {

		// Send a message that should end up in a single frame
		...
		testMessage1 = (testMessage3Len + testMessage3Str + testMessage4Len + testMessage4Str)
	...
	public void handleConnectionClose() {
		...
		case customer:
	        ...
	        // TODO: If customer quit, tell all agents that customer is gone
		    for (User agent : server.getAgents()) {
